...
void belief_moments(double* p, double* moments, double* S){
  ...
  *S=0.;
  ...
  for(i=0;i<NX;i++){
	...
	for(n=0;n<2;n++){
	  moments[3*n]+=p[k]*pow(i,n+1);
	  moments[3*n+1]+=p[k]*pow(j,n+1);
	  moments[3*n+2]+=p[k]*pow(l,n+1);
	}
      }
    }
  }
...
string get_name_params_specific(const int Nepis, const int n){
    ...
    if(different_weights){
      for(iag=0;iag<n_info;iag++)
        a += "0.00_";
      for(iag=n_info;iag<Nagents;iag++)
        a += "1.00_";
    }
    else{
      for(iag=0;iag<Nagents;iag++)
...
vec3d place_from_hits() {
    ...
    int width=5, lowx, highx, lowy, highy, lowz, highz;
