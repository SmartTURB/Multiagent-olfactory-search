...
//Initialize problem
void init_problem(){
  int iag,temp1,temp2,temp3,temp4,temp5,temp6,temp7,iread;
  FILE *fpara;
  string fname;
  ...
  //Implementing random initialization or not
  iread=fscanf(fpara,"%d %d",&temp6,&temp7);
  rand_start=temp6;
  start_no_hit=temp7;
  ...
  bmoments = (double*) malloc(sizeof(double)*6);
...
string init_agents_source_belief_hits(int iep, int times, int DeltaT,int *tStart){
  ...
  *tStart = (iep%times)*tOverlap + (int)(DeltaT*drand48()); //Start from random time
  ...
  if(given_rA.x<0){ //From a position with a hit                                                                                        
    ...
    newPos:
      ...
      if(manhattan(agent[0].rA.x,agent[0].rA.y,agent[0].rA.z,source.rS.x,source.rS.y,source.rS.z)<=5){ //Condition initial agents pos
	iterPos+=1;
	if(iterPos>10)
	  goto newTime;
	goto newPos;
    ...
    agent[0].rA = given_rA; //In a fixed position 
  
  //Place the remaining agents    
  place_multi_agents();
