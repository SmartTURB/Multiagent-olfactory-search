...
void place_multi_agents(){
  ...
  int border = 15;
  double g, n, rw[Nagents];

  g = (double) (Nagents - n_info);
  n = (double) Nagents;
  ...
  if(rand_start){
    ...
    if(different_weights){
      for(iag=0;iag<Nagents;iag++){
        ...
        n--;
        cout << "weight agent"<< iag << " is " << agent[iag].weight << endl;
      }
    }

    i0 = (start_no_hit ? 0 : 1);
    
    //Place randomly or in a plane the rest of the agents
    for(iag=i0;iag<Nagents;iag++){ //IF iag starts from 0, all agents start randomly
    tryAgain:
      agent[iag].rA.x = border + (int) (NX-2*border)*drand48();
      agent[iag].rA.y = border + (int) (NY-2*border)*drand48();
      agent[iag].rA.z = border + (int) (NZ-2*border)*drand48();
      
      //Exclude source region
      if(manhattan(agent[iag].rA.x,agent[iag].rA.y,agent[iag].rA.z,source.rS.x,source.rS.y,source.rS.z)<=5)
	goto tryAgain;
    } 
  }
  else{  
